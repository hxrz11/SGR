...
async function submitQuery() {
    ...
    resetSteps();
    addStep('Отправка запроса к модели...');
    ...
    try {
        ...
        addStep('Ответ получен от модели');
...
function displayResults(result, preserveSteps = false) {
    if (!preserveSteps) {
        resetSteps();
    }
    ...
    if (thinkMatch) {
        document.getElementById('thought-text').textContent = thinkMatch[1].trim();
        document.getElementById('thought-card').style.display = 'block';
        document.getElementById('final-answer-text').textContent = answer.replace(thinkMatch[0], '').trim();
    } else {
        document.getElementById('thought-card').style.display = 'none';
        document.getElementById('final-answer-text').textContent = answer;
    }
